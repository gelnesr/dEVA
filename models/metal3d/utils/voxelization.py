...
def voxelize_single_notcentered(env):
    ...
    voxels = torch.from_numpy(prot_vox_t.copy()).float()
...
def processStructures(pdb_file, resids, clean=True):
    ...
    results = [] #p.imap(voxelize_single_notcentered, environments) # prot_centers, prot_N
    
    for env in environments:
        res = voxelize_single_notcentered(env)
        if res is not None:
            results.append(res)

    if len(results) == 0:
        exit("something went wrong with the voxelization, check that there are no discontinuities in the protein")

    vox_env, prot_centers_list, prot_n_list, envs = zip(*results)
    voxels = torch.stack(vox_env, dim=0)
